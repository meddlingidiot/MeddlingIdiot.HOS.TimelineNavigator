...
namespace Automation.HOS.TimelineNavigator.UnitTests
{
    ...
    {
        [Test]
        public void StartOfDay_automatically_added_and_intermingles_with_GPS_and_EngineBus()
        {
            var sut = new TimelineNavigator(new StartOfDayTimelineOptions(daysToLoadBeforeEarliestTimestamp: 0));
            ...
            Assert.That(moments[1].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 00:00:00"))); // Auto-generated StartOfDay at midnight
            Assert.That(moments[2].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 05:58:00"))); // GPS
            Assert.That(moments[3].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 05:59:00"))); // EngineBus
            Assert.That(moments[4].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 06:01:00"))); // EngineBus
            Assert.That(moments[5].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 06:02:00"))); // GPS

            // Verify types - StartOfDay is auto-generated at midnight
            Assert.That(moments[1], Is.TypeOf<StartOfDayMoment>());
            Assert.That(moments[2], Is.TypeOf<GpsMoment>());
            Assert.That(moments[3], Is.TypeOf<EngineBusMoment>());
            Assert.That(moments[4], Is.TypeOf<EngineBusMoment>());
            Assert.That(moments[5], Is.TypeOf<GpsMoment>());
        ...
        [Test]
        public void GPS_and_EngineBus_data_intermingle_perfectly_during_truck_route()
        {
            var sut = new TimelineNavigator(new StartOfDayTimelineOptions(daysToLoadBeforeEarliestTimestamp: 0));
            ...
            // Collect all moments in order
            // Pull them out of the enumerator
            var moments = new List<Moment>();
            foreach (var moment in sut)
            {
                moments.Add(moment);
            ...
            for (int i = 1; i < moments.Count; i++)
            {
                ...
                Assert.That(moments[i].Timestamp, Is.GreaterThanOrEqualTo(moments[i-1].Timestamp),
                    $"Moment {i} at {moments[i].Timestamp} should be >= moment {i-1} at {moments[i-1].Timestamp}");
                
                if (i == 1)
                {
                    Assert.That(moments[i].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 00:00:00")));
                }
                else if (i == 2)
                {
                    Assert.That(moments[i].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 06:00:00")));
                }
                else if (i == 3)
                {
                    Assert.That(moments[i].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 06:15:00")));
                }
                else if (i == 3)
                {
                    Assert.That(moments[i].Timestamp, Is.EqualTo(DateTime.Parse("01/15/2024 06:30:00")));
                }
            }
            ...
            Assert.That(gpsMoments.Count, Is.EqualTo(24));
            Assert.That(engineBusMoments.Count, Is.EqualTo(5));
            Assert.That(moments.Count, Is.EqualTo(31));

            // Verify GPS coordinates are realistic
            var firstGps = gpsMoments.First();
            var lastGps = gpsMoments.Last();
            sut.JumpTo(DateTime.Parse("01/15/2024 06:00:00"));
            Assert.That(sut.Latitude, Is.EqualTo(34.0522).Within((0.01))); // Los Angeles
            Assert.That(sut.Longitude, Is.EqualTo(-118.2437).Within((0.01)));
            Assert.That(sut.Odometer, Is.EqualTo(125000m));
            sut.JumpTo(DateTime.Parse("01/15/2024 14:30:00"));
            Assert.That(sut.Latitude, Is.EqualTo(37.7849).Within((0.01))); // San Francisco
            Assert.That(sut.Longitude, Is.EqualTo(-122.4094).Within((0.01)));
            Assert.That(sut.Odometer, Is.EqualTo(125385m));
            ...
            Assert.That(hasIntermingling, Is.True, "GPS and EngineBus moments should be intermingled");
        ...
        [Test]
        public void GPS_and_EngineBus_with_simultaneous_timestamps_maintain_order()
        {
            var sut = new TimelineNavigator(new StartOfDayTimelineOptions(stopAtStartOfDay: false, daysToLoadBeforeEarliestTimestamp: 0));
            ...
            Assert.That(gpsMoments.Count, Is.EqualTo(5));
            Assert.That(engineBusMoments.Count, Is.EqualTo(0));
            Assert.That(moments.Count, Is.EqualTo(6));
            ...
            Assert.That(sut.Start.Timestamp, Is.EqualTo(DateTime.Parse("01/16/2024 09:00:00")));
            Assert.That(sut.Odometer, Is.EqualTo(87500m));

            sut.JumpTo(DateTime.Parse("01/16/2024 11:00:00"));
            Assert.That(sut.Start.Timestamp, Is.EqualTo(DateTime.Parse("01/16/2024 11:00:00")));
            Assert.That(sut.Odometer, Is.EqualTo(87625m));
        }
    }
}
